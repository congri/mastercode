...
if(strcmp(domain.basisFunctionType,'polynomial'))
    for i = 1:conductivity.Nx
        for j = 1:conductivity.Ny
            ...
            gradK(:,:,l) = get_glob_stiff2(domain, grad_kl(:,:,:,l));
...
% pref = zeros(conductivity.dim, domain.nElements);
% for k = 1:domain.nElements
%     pref(:, k) = exp(a + domain.exponent(k,:)');
%     for l = 1:conductivity.dim
%         grad_kl(:, :, k, l) =pref(l, k)*conductivity.localStiffnessOffset;
%     end
% end
% 
% gK.grK = sparse(SK(1), SK(2), 3*SK(1));
% gK = repmat(gK, 1, conductivity.dim);
% for l = 1:conductivity.dim
%     gK(l).grK = get_glob_stiff2(domain, grad_kl(:,:,:,l));
% end
